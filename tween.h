...
template <typename T> struct TweenAttr : public TweenAttrBase
{
    ...
    TweenAttr(T* target, T value, int cycles)
        : startValue(*target), delta(value - *target),
          maxValue(std::numeric_limits<T>::max()), target(target)
    {
        if (cycles != 1) {
            maxValue = delta + 1;
...
public:
    ...
    // Create a new empty Tween. It will start automatically when destroyed, if
    // it was not started manually. This allows you do chain the call;
    // `make().to(...).delay()` etc.
    static Tween make();
    ...
    // template <typename T> is_compound<T, Tween&> to(T &target, T value, int
    // cycles = 1) { 	for(int i=0; i<target.size(); i++) { 		to(target[i],
    // value[i], cycles);
    // 	}
    // 	return *this;
    // }
    ...
    template <typename T>
    is_compound<T, Tween&> to(T& target, T value, int cycles = 1)
    {
    ...
    TweenT<T> to(T& target, U value, int cycles = 1)
    {
        ...
        auto taptr = std::make_shared<TweenAttr<T>>(
            targetp, static_cast<T>(value), cycles);
...
// A TweenT is a Tween that currently references a tweenable value. You can for
// instance only call onUpdate() on this kind of Tween, since it can refer to
// the last value added to the Tween implementation.
template <typename T> class TweenT : public Tween
{
public:
    TweenT(std::shared_ptr<TweenImpl> i, std::shared_ptr<TweenAttr<T>> ta)
        : Tween(i), ta(ta)
    {}

    TweenT(const TweenT& t) { impl = t.impl; }
